...
import { PDFExportProps } from './ExportTypes';
import { captureChartAsImage, formatTimestamp } from '../ExportUtils';
...
const PDFExport = forwardRef<HTMLButtonElement, PDFExportProps>((props, ref) => {
  ...
  const exportToPDF = async () => {
    ...
    callbacks.setModalTitle('Exportando a PDF');
    callbacks.setModalContent('Generando reporte PDF con gráficos y datos filtrados...');
    ...
    try {
      ...
      const addMetrics = (yPosition: number) => {
        ...
        const metricsData = [
          ...
          ['Eventos Filtrados', filteredEventsCount.toString()]
        ];
      ...
      callbacks.setModalContent(`El reporte PDF se ha generado exitosamente con ${filteredEvents.length} eventos.`);
    } catch (error) {
      console.error('Error al generar PDF:', error);
      // Mostrar modal de error
      callbacks.setModalLoading(false);
      callbacks.setModalTitle('Error en Exportación');
      callbacks.setModalContent('No se pudo generar el reporte PDF. Por favor, intente nuevamente.');
    }
  };
  ...
  return (
    <button ref={ref} onClick={exportToPDF} style={{ display: 'none' }}>
      Exportar PDF
    </button>
  );
});
